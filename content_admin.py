"""پنل ادمین محتوا — علوم پایه، رفرنس‌ها، FAQ"""
import os, logging
...
async def content_admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # تشخیص منبع (از پنل ادمین اصلی یا پنل محتوا)
    came_from_admin = action.endswith('_admin')
    back_to_main    = 'admin:main' if came_from_admin else 'ca:main'

    # ── منو ──
    if action == 'main':
        await _ca_main(query, back='')

    # ── علوم پایه ──
    elif action in ('terms', 'terms_admin'):
        context.user_data['ca_from_admin'] = came_from_admin
        await _ca_show_terms(query, back=back_to_main)

    elif action == 'term':
        idx = int(parts[2])
        context.user_data['ca_term']     = TERMS[idx]
        context.user_data['ca_term_idx'] = idx
        from_admin = context.user_data.get('ca_from_admin', False)
        back = f'ca:terms_admin' if from_admin else 'ca:terms'
        await _ca_show_lessons(query, context, TERMS[idx], back=back)
        ...
        await query.edit_message_text(
            ...
            "نام درس و استاد را بنویسید:\n"
            "<i>مثال: فیزیولوژی, دکتر احمدی</i>\n<i>(استاد اختیاری)</i>",
            parse_mode='HTML',
        ...
        await query.edit_message_text(
            ...
            f"شماره پیشنهادی: <b>{next_num}</b>\n\n"
            "اطلاعات را وارد کنید:\n<i>فرمت: شماره, موضوع, استاد</i>\n"
        ...
        await query.edit_message_text("📤 <b>نوع محتوا را انتخاب کنید:</b>",
                                       parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))

    elif action == 'sel_ctype':
        session_id = parts[2]; ctype = parts[3]
        context.user_data['ca_session_id']   = session_id
        ...
        await query.edit_message_text(f"📤 <b>آپلود {type_label}</b>\n\nفایل را ارسال کنید:",
                                       parse_mode='HTML',
        ...
        if item:
            ...
            await query.edit_message_text(
                f"⚠️ حذف: {type_label} — {item.get('description','')[:30]}?",
                reply_markup=InlineKeyboardMarkup([
    ...
    # ── رفرنس‌ها ──
        ...
        context.user_data['ca_ref_from_admin'] = came_from_admin
        await _ca_ref_subjects(query, back=back_to_main)

    elif action == 'add_ref_subject_prompt':
        context.user_data['ca_mode'] = 'add_ref_subject'
        from_admin = context.user_data.get('ca_ref_from_admin', False)
        back = 'ca:refs_admin' if from_admin else 'ca:refs'
        await query.edit_message_text("➕ <b>درس جدید برای رفرنس</b>\n\nنام درس را بنویسید:",
                                       parse_mode='HTML',
        ...
        if subj:
            from_admin = context.user_data.get('ca_ref_from_admin', False)
            back = 'ca:refs_admin' if from_admin else 'ca:refs'
            await query.edit_message_text(f"⚠️ حذف درس «{subj['name']}» و تمام رفرنس‌هایش؟",
                reply_markup=InlineKeyboardMarkup([
        ...
        from_admin = context.user_data.get('ca_ref_from_admin', False)
        back = 'ca:refs_admin' if from_admin else 'ca:refs'
        await query.edit_message_text("✅ درس رفرنس حذف شد.",
        ...
        from_admin = context.user_data.get('ca_ref_from_admin', False)
        back = 'ca:refs_admin' if from_admin else 'ca:refs'
        await _ca_ref_books(query, context, subject_id, back=back)

    elif action == 'add_ref_book_prompt':
        subject_id = parts[2]
        context.user_data['ca_ref_subject_id'] = subject_id
        context.user_data['ca_mode']           = 'add_ref_book'
        await query.edit_message_text("➕ <b>رفرنس جدید</b>\n\nنام کتاب را بنویسید:",
                                       parse_mode='HTML',
        ...
        if book:
            ...
            await query.edit_message_text(f"⚠️ حذف رفرنس «{book['name']}»؟",
                reply_markup=InlineKeyboardMarkup([
    ...
    # ── FAQ ──
        ...
        await query.edit_message_text(
            ...
            "مثال: <code>نحوه دانلود? | روی دکمه دانلود کلیک کنید | ⚙️ مشکلات فنی</code>",
            parse_mode='HTML',
...
async def _ca_show_terms(query, back='ca:main'):
    keyboard = []
    ...
    await query.edit_message_text("📘 <b>انتخاب ترم</b>", parse_mode='HTML',
                                   reply_markup=InlineKeyboardMarkup(keyboard))


async def _ca_show_lessons(query, context, term, back='ca:terms'):
    ...
    keyboard.append([InlineKeyboardButton("🔙 بازگشت", callback_data=back)])
    await query.edit_message_text(f"📘 <b>{term}</b> — {len(lessons)} درس",
                                   parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))
...
async def _ca_show_sessions(query, context, lesson_id):
    ...
    name = lesson.get('name','') if lesson else ''
    await query.edit_message_text(f"📖 <b>{name}</b> — {len(sessions)} جلسه",
                                   parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))
...
async def _ca_show_session_content(query, context, session_id):
    ...
    await query.edit_message_text(f"📌 <b>جلسه {snum} — {stopic}</b>\n{len(contents)} فایل:",
                                   parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))


async def _ca_ref_subjects(query, back='ca:main'):
    ...
    keyboard.append([InlineKeyboardButton("🔙 بازگشت", callback_data=back)])
    await query.edit_message_text(f"📚 <b>رفرنس‌ها</b> — {len(subjects)} درس",
                                   parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))


async def _ca_ref_books(query, context, subject_id, back='ca:refs'):
    subj  = await db.ref_get_subject(subject_id)
    ...
    await query.edit_message_text(f"📖 <b>{name}</b> — {len(books)} رفرنس:",
                                   parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))
...
async def _ca_faq_manage(query):
    ...
    await query.edit_message_text(
        ...
        "⚠️ اگر خالی باشد، سوالات پیش‌فرض ربات نمایش داده می‌شود.",
        parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))
...
async def ca_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ca_mode == 'add_lesson':
        ...
        name    = parts_list[0]
        teacher = parts_list[1] if len(parts_list) > 1 else ''
        term    = context.user_data.get('ca_term','')
        result  = await db.bs_add_lesson(term, name, teacher)
        await update.message.reply_text("✅ درس اضافه شد!" if result else "⚠️ این درس قبلاً وجود دارد.")
        context.user_data['ca_mode'] = ''
        ...
        if len(parts_list) < 2:
            ...
            return CA_WAITING_TEXT
        try:    number = int(parts_list[0])
        except: number = len(await db.bs_get_sessions(context.user_data.get('ca_lesson_id',''))) + 1
        topic   = parts_list[1]
        teacher = parts_list[2] if len(parts_list) > 2 else ''
        lesson_id = context.user_data.get('ca_lesson_id','')
        ...
        await update.message.reply_text("✅ درس رفرنس اضافه شد!" if result else "⚠️ این درس قبلاً وجود دارد.")
        context.user_data['ca_mode'] = ''
        ...
        parts_list = [p.strip() for p in text.split('|')]
        if len(parts_list) < 2:
            await update.message.reply_text("❌ فرمت اشتباه. مثال: سوال؟ | جواب | دسته")
            return CA_WAITING_TEXT
        question = parts_list[0]
        answer   = parts_list[1]
        category = parts_list[2] if len(parts_list) > 2 else 'عمومی'
        await db.faq_add(question, answer, category)
        await update.message.reply_text(f"✅ سوال در دسته «{category}» اضافه شد!")
        context.user_data['ca_mode'] = ''
